...
$process = Start-Process -FilePath $pythonExe -ArgumentList "`"$appScript`"" -PassThru -WindowStyle Normal
 
# Wait and verify API is serving
...
for ($i = 1; $i -le 15; $i++) {
  ...
  try {
    $response = Invoke-WebRequest -Uri "http://127.0.0.1:$PORT/api/health" -UseBasicParsing -TimeoutSec 2 -ErrorAction SilentlyContinue
...
if (-not $ready) {
  Write-Host ""
  Write-Host "[!!] Could not confirm API readiness after 15 seconds." -ForegroundColor Yellow
  Write-Host "     The API may still be starting. Checking the process..." -ForegroundColor Yellow
  if (-not $process.HasExited) {
    Write-Host "[OK] API process is still running (PID $($process.Id))." -ForegroundColor Green
    Write-Host "     It may just be slow to respond. Opening dashboard anyway." -ForegroundColor Yellow
    Write-Host "     If the dashboard does not load, wait a few seconds and refresh." -ForegroundColor Yellow
  } else {
    Write-Host "[XX] API process has stopped. Run manually to see the error:" -ForegroundColor Red
    Write-Host "     & `"$pythonExe`" `"$appScript`"" -ForegroundColor White
    exit 1
  }
}
...
Write-Host "   .\monitor-start.ps1" -ForegroundColor White
Write-Host ""
